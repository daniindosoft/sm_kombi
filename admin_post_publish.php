<?php
  require('header.php');
  $sql = 'select count(id) as total from post where id_komunitas_bisnis='.$_SESSION['bisnis_kategori_combi'].' and ';
  
  $sql2 = 'select count(id) as total from post where id_komunitas_bisnis='.$_SESSION['bisnis_kategori_combi'].' and ';

  $jmlPost = $onMy->eksekusiShow($sql.' id_user='.$_COOKIE['id_akun_combi'].' and status= "publish" ')['total'];
  $jmlPostDraft = $onMy->eksekusiShow($sql.' id_user='.$_COOKIE['id_akun_combi'].' and status= "draft" ')['total'];
  $jmlPostDelay = $onMy->eksekusiShow($sql.' status= "delay" ')['total'];

  $jmlPostAllDelay = $onMy->eksekusiShow($sql2.' status= "delay" ')['total'];

  $status = $_GET['status'];
  $onMy->isAdminDelay = true;
  if ($_GET['status'] == 'publish' || empty($_GET['status'])) {
    $status = 'publish';
            ...
            <div class="card">
              ...
              <div class="card-body p-0">
                <ul class="nav nav-pills flex-column">
                  ...
                  <?php endif ?>
                  <li class="nav-item <?php echo ($_GET['status'] == 'draft') ? 'active':'' ?>" id="jumlah-draft">
            ...
            <?php 
              ...
              foreach ($loopPost as $value): 
                ...
                if ($value['status'] == 'publish') {
                  $badge = '<span class="badge badge-warning">Publish</span>';
                }elseif($value['status'] == 'delay'){
                  $badge = '<span class="badge badge-success">Belum di verifikasi</span>';

                }elseif($value['status'] == 'draft'){
                  $badge = '<span class="badge badge-info">Draft</span>';
            ...
            ?>
              <div class="card card-widget collapsed-card">
                <div class="card-header">
                  ...
                  <div class="card-tools">
                    <?php if ($_GET['status'] == 'delay'): ?>
                      <a href="#" onclick="return redirect('<?php echo $onMy->primaryLocal ?>admin/post/acc?url=<?php echo $onMy->thisUrl(); ?>&id=<?php echo $value['id'] ?>', 'q')" class="float-right btn btn-sm btn-warning"><i class="fa fa-check"></i> Publish</a>
                    <?php endif; ?>
                    <?php if ($_GET['status'] == 'draft'): ?>
                    <button type="button" class="btn btn-tool" data-card-widget="collapse">
                    ...
                    <button type="button" class="btn btn-tool" data-card-widget="collapse">
                      ...
                      <a onclick="return confirm('Yakin menghapus Postingan ini ?, Komentar dan like akan hilang juga')" href="<?php echo $onMy->primaryLocal ?>admin/hapus?id=<?php echo $value['id'] ?>&table=<?php echo base64_encode('post') ?>&url=<?php echo $onMy->thisUrl() ?>&tolak=ok" class="btn btn-tool">
