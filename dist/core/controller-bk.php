<?php
	...
	if(isset($_POST['submitBisnisKomunitas'])) {
		...
		$col=array('nama_komunitas','nowa','website','ig','tiktok','komisi_affiliate_join', 'code_komunitas', 'id_user', 'id_kategori_bisnis', 'member_show', 'verifikasi_post', 'harga', 'note', 'wa_template', 'is_dropship', 'is_affiliate');
			...
			if ( !empty($cekTf) ){
				...
				if (empty($id_komunitas)) {
					...
					$text .= '<div class="alert alert-info"><b>Selamat</b> !, Anda segera bergabung ke komunitas baru, segera ikuti langkah dibawah ini.</div>';
		          	...
		          	// post notifikasi
					// $user = $sistem->thisProfile($_COOKIE['bisnis_kategori_combi']);
					// $sistem->postNotifikasi( $_COOKIE['bisnis_kategori_combi'], $id_komunitas['idu'], 'aff_join', $user['nama_lengkap'], '<b>'.$user['nama_lengkap'].'</b> daftar komunitas lewat link affiliate Anda', $owner['idk'] );
				}
			}
